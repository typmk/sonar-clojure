(ns au.com.heisenbergtech.sonar.plugin-test
...
(deftest language-claims-clojure-extensions
  (let [lang (instantiate "au.com.heisenbergtech.sonar.ClojureLanguage" (config {}))]
    (is (= "clj" (.getKey lang)))
    ...
    (testing "and are overridable, which is why the ctor takes Configuration"
      (let [l (instantiate "au.com.heisenbergtech.sonar.ClojureLanguage"
...
(deftest rules-definition-registers-every-kondo-linter
  (let [ctx  (RulesDefinition$Context.)
        _    (.define (instantiate "au.com.heisenbergtech.sonar.ClojureRulesDefinition") ctx)
...
(deftest plugin-registers-all-extensions
  (let [ctx (Plugin$Context. (runtime))]
    (.define (instantiate "au.com.heisenbergtech.sonar.ClojurePlugin") ctx)
    (let [exts    (.getExtensions ctx)
      ...
      (testing "every extension class the plugin declares is registered"
        (is (= #{"au.com.heisenbergtech.sonar.ClojureLanguage"
