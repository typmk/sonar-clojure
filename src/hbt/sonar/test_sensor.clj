...
(defn -execute [_ ctx]
  (doseq [^File f (report/paths ctx const/test-report-paths-prop const/default-test-report)]
    (if (report/exists? f)
      (import-report! ctx f)
